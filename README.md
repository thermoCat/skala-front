...
SKALA Frontend 과정에서 내준 HTML·CSS·JS 과제를 낱개 파일로 흩어두는 대신,
로그인 한 번으로 전부 이어지는 **개인 포털 하나**로 묶어봤다. 서버 없이 정적 파일과
브라우저 저장소(`localStorage`)만으로 회원가입, 개인화, 데이터 저장까지 전부 처리한다.

**🔗 배포 주소: [skala-front-jet.vercel.app](https://skala-front-jet.vercel.app/)**
— 메인 Hub는 로그인 전엔 화면이 흐리게 가려져 있으니, 회원가입부터 먼저 해보면 된다.
...
배포는 Vercel을 쓴다. 실제 페이지는 전부 `html/` 아래에 있어서, 루트(`/`)와
